...
public class AddressController {
	...
	public Result add(@RequestBody TbAddress address) {
		...
		// 将数据库中原有的默认地址改为不是默认地址
		if ("1".equals(address.getIsDefault())) {
			List<TbAddress> addressList = addressService.findByUserId(userId);
			for (int i = 0; i < addressList.size(); i++) {
				TbAddress addressInDataBase = addressList.get(i);
				if ("1".equals(addressInDataBase.getIsDefault())) {
					addressInDataBase.setIsDefault("0");
					addressService.update(addressInDataBase);
					break;
				}
			}
		}
